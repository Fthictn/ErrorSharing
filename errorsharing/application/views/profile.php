<!DOCTYPE html>
 ...
 <head>
     ...
     <title><?php if(@$_SESSION["name"] == "admin") { ?> Errors
     <?php } else { ?> Profile <?php } ?>
     </title>
 </head>
                    ...
                    <div id="content">
                                ...
                                <div id="icerik">
                                    ...
                                    <?php foreach ($myerrors as $error) { ?>
                                        <div id="sharing">
                                            <form id="updateerror" role="form" onsubmit="return false;" method="post" name="errorForm">
                                                <div><input type="hidden" name="user_id" value="<?=$_SESSION["id"]?>"></div>
                                                <div><input type="text" id="subject<?=$error->postId?>" name="subject<?=$error->postId?>" placeholder="Subject" value="<?=$error->topic?>"></div>
                                                <div><button onclick="sil(<?=$error->postId?>)" style="color:red;font-size: 20px">Sil</button></div>
                                                <div><textarea class="textarea1" id="problem<?=$error->postId?>" name="problem<?=$error->postId?>" placeholder="Problem"><?=$error->error?></textarea></div>
                                                <div><textarea class="textarea1" id="answer<?=$error->postId?>" name="answer<?=$error->postId?>" placeholder="Answer"><?=$error->solution?></textarea></div>
                                                <div><button onclick="kontrol(<?=$error->postId?>);" id="shareerrorbutton">Update</button></div>
                                            </form>
                                        </div>  
                                    <?php } ?>
                                <?php }else{ ?>
